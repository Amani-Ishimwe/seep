  ...
  return (
    <div className="flex flex-col select-none max-w-5xl mx-auto py-2 text-lowercase text-[#0a0a0a] gap-6">
      ...
      <div className="text-left flex justify-between items-end">
        <div>
          <span className="text-[9px] font-bold uppercase tracking-[0.15em] text-[#8e8e93] block mb-1">leakage logs</span>
          <h1 className="text-2xl font-black tracking-tight text-[#0a0a0a]">unbilled leak investigations</h1>
        </div>
        <div className="text-right">
          <span className="text-[9px] uppercase tracking-wider font-bold text-[#8e8e93] block">recovered this month</span>
          <span className="text-xl font-black text-[#006622]">{formatCurrency(recoveredThisMonth)}</span>
        </div>
      </div>

      {/* Scope Creep Alert Banner */}
      {clientLeakage.length > 0 && (
        <div className="bg-[#a94442]/5 border border-[#a94442]/10 rounded-xl p-5 flex justify-between items-center text-left">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 bg-[#a94442]/10 rounded-lg flex items-center justify-center text-[#a94442] shrink-0">
              <i className="fa-solid fa-circle-exclamation text-[18px]"></i>
            </div>
            <div>
              <span className="text-xs font-bold text-[#0a0a0a] block">scope creep warning detected</span>
          ...
          <button 
            ...
            className="px-4 py-2 bg-[#0a0a0a] text-white text-[10px] font-bold uppercase tracking-wider rounded-lg hover:bg-black/85 transition-colors shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5 items-start">
        ...
        <div className="lg:col-span-2 flex flex-col gap-3">
          <h3 className="text-xs font-bold uppercase tracking-[0.1em] text-[#8e8e93] text-left">detected unbilled activity</h3>
          
          {detectedLeaks.length === 0 ? (
            <div className="card rounded-xl p-10 text-center flex flex-col items-center">
              <div className="w-12 h-12 rounded-xl bg-[#f2f2f2] flex items-center justify-center mb-4">
                <i className="fa-solid fa-check text-xl text-[#8e8e93]"></i>
              </div>
              <h3 className="text-sm font-bold text-[#0a0a0a] mb-1">no leaks detected yet</h3>
            ...
            detectedLeaks.map((evt) => {
              ...
              return (
                <div key={evt.id} className="card rounded-xl p-5 text-left flex flex-col justify-between gap-4">
                  ...
                  <div className="border-t border-[#e0e0e0] pt-3.5 flex flex-col gap-3">
                    ...
                    <div className="flex justify-between items-center bg-[#fafafa] p-3 border border-[#e0e0e0] rounded-lg">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#0a0a0a] text-white text-[9px] font-bold uppercase tracking-wider rounded-md hover:bg-black/85 transition-colors"
                        >
                          confirm
                        </button>
                        <select
                          ...
                          className="px-2 py-1 text-[9px] bg-white border border-[#e0e0e0] rounded-md outline-none focus:border-[#0a0a0a] font-sans font-bold text-[#555555]"
                        >
                        ...
                        <button
                          ...
                          className="px-2.5 py-1 bg-[#f2f2f2] text-[#8e8e93] border border-[#e0e0e0] text-[9px] font-bold uppercase tracking-wider rounded-md hover:text-[#0a0a0a] hover:border-[#0a0a0a]/20 transition-colors"
                        >
                          ignore
                        </button>
                      </div>
        ...
        <div className="flex flex-col gap-6">
          <h3 className="text-xs font-bold uppercase tracking-[0.1em] text-[#8e8e93] text-left">unbilled by client</h3>
          
          <div className="card rounded-xl p-5 text-left flex flex-col gap-3">
            {clientLeakage.length === 0 ? (
              <span className="text-xs text-[#8e8e93] py-4 text-center">no client leaks logged.</span>
            ) : (
              clientLeakage.map(({ client, duration, amount }) => (
                <div key={client.id} className="flex justify-between items-center py-3 border-b border-[#e0e0e0] last:border-0 last:pb-0">
                  <div>
                  ...
                  <div className="text-right flex flex-col items-end gap-1.5">
                    ...
                    <button 
                      ...
                      className="px-2 py-1 bg-[#006622]/10 border border-[#006622]/20 text-[#006622] rounded-md text-[8px] font-bold uppercase tracking-widest hover:bg-[#006622]/20 transition-all"
                    >
                      mark recovered
                    </button>
                  </div>
      ...
      {activeDraftClient && (
        <div className="fixed inset-0 z-50 bg-[#0a0a0a]/15 backdrop-blur-sm flex justify-center items-center p-6 animate-fade-in">
          <div className="card w-full max-w-[460px] p-8 rounded-xl text-left relative">
            <button 
            ...
            <div className="bg-[#fafafa] border border-[#e0e0e0] rounded-lg p-4 text-[11px] font-mono text-[#555555] whitespace-pre-line leading-relaxed mb-6 select-text">
              {generateEmailText(
                activeDraftClient,
            ...
            <button
              ...
              className="w-full py-3 bg-[#0a0a0a] text-white text-xs font-bold uppercase tracking-wider rounded-lg hover:bg-black/85 transition-colors"
            >
